...
public class AdministrativeController {
    ...
    public ModelAndView showFormSchedule(){
        ModelAndView modelAndView = new ModelAndView(Pages.ADMIN_EDIT_SCHEDULE);
        modelAndView.addObject("user",userService.getUser());
        modelAndView.addObject("subjects",subjectDAO.getTableList());
        return modelAndView;
    }

    @PostMapping("/new-schedule-subject")
    public ModelAndView createNewSchedule(@RequestParam("subject") Long subjectId, @RequestParam("day") int day, @RequestParam("durability") Integer durability, @RequestParam("start") String start, @RequestParam("end") String end) {
       administrativeService.getScheduleParameters(subjectId,day,durability,start,end);
